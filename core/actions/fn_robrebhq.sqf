...
_chance = random(100);
if(_chance >= 85) then { hint "The rebels hit the silent alarm, other rebel factions have been alerted!"; [[1,format["ALARM! - CSAT RebelHQ: %1 is being robbed!", _shop]],"life_fnc_broadcast",west,false] spawn life_fnc_MP; };
...
[[1,format["CSAT RebelHQ: %1 was just robbed by %2 for a total of $%3", _shop, _robber, [_kassa] call life_fnc_numberText]],"life_fnc_broadcast",west,false] spawn life_fnc_MP;
_rip = false;
life_use_atm = false;
sleep (30 + random(180));
life_use_atm = true;
if!(alive _robber) exitWith {};
[[1,format["NEWS: The CSAT Rebels: %1 was just robbed for a total of $%2", _shop, [_kassa] call life_fnc_numberText]],"life_fnc_broadcast",civilian,false] spawn life_fnc_MP;
